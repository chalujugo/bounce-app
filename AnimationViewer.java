...
public class AnimationViewer extends JComponent implements Runnable {
    ...
    protected void createNewShape(int x, int y) {
        switch (currentShapeType) {
			...
			case NESTED: {
			    treeModelAdapter.addToRoot(new NestedShape(x,y,currentWidth,currentHeight,marginWidth,marginHeight,currentBorderColor,currentFillColor,currentPathType));
			}
		}
    ...
    public boolean addShape(TreePath selectedPath) {
        Object obj = selectedPath.getLastPathComponent();
        if(treeModelAdapter.addNode(selectedPath, currentShapeType)) {
          tableModelAdapter.fireTableDataChanged();
          return true;
        }  else{
            return false;
